...
def one_to_many(X, y, generators, train_gen):
    real_idx = np.where(y==1)[0]
    train_fake = np.where(generators == train_gen)[0]

    rng = np.random.default_rng(42)
    real_train = rng.choice(real_idx, int(len(real_idx) * 0.7), replace=False)
    real_test = np.array([i for i in real_idx if i not in set(real_train)])
    test_fake = np.where((y==0) & (generators != train_gen))[0]
    ...
    return {"train_gen": GEN_SHORT[train_gen], "auc": roc_auc_score(y[te], probs), "acc": accuracy_score(y[te], preds), "n_train": len(tr), "n_test": len(te)}


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--train-gen", default=None)
    ...
    train_gens = [args.train_gen] if args.train_gen else fake_gens


    results = []
    for g in train_gens:
        r = one_to_many(X, y, generators, g)
        print(f" train={r['train_gen']:>8s}: AUC={r['auc']:.3f}, Acc={r['acc']:.3f}")
        results.append(r)

    aucs = [r["auc"] for r in results]
    accs = [r["acc"] for r in results]
    print(f"mean AUC: {np.mean(aucs):.3f}, mean Acc: {np.mean(accs):.3f}")


    if args.save_csv:
        out = Path(__file__).resolve().parent.parent / "results" / "comparison_protocols"
        out.mkdir(parents=True, exist_ok=True)
        pd.DataFrame(results).to_csv(out / "one_to_many.csv", index=False)
        
        print(f"saved to {out / 'one_to_many.csv'}")
