...
def stratified_split(y, generators, video_ids, seed=42):
    """70/15/15 split stratified by class+generator"""
    ...
    if len(unique_vids)<n:
        ...
        tr_v, tmp_v = train_test_split(vid_idx, test_size=0.30, stratify=unique_strat, random_state=seed)
        va_v, te_v = train_test_split(tmp_v, test_size=0.50, stratify=unique_strat[tmp_v], random_state=seed)
        tr_vids = set(unique_vids[tr_v])
        va_vids = set(unique_vids[va_v])
        te_vids = set(unique_vids[te_v])
        tr_idx = np.array([i for i in idx if video_ids[i] in tr_vids])
        va_idx = np.array([i for i in idx if video_ids[i] in va_vids])
        te_idx = np.array([i for i in idx if video_ids[i] in te_vids])
    else:
        tr_idx, tmp_idx = train_test_split(idx, test_size=0.30, stratify=strat_labels, random_state=seed)
        va_idx, te_idx = train_test_split(tmp_idx, test_size=0.50, stratify=strat_labels[tmp_idx], random_state=seed)
